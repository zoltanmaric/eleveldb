...
// shift less ... someday use CLOCK_MONOTONIC_RAW
    ...
    ERL_NIF_TERM profile(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
    {
        try {
            ...
            if(atom == "noop") {
                nifutil::Profiler::noop(ErlUtil::getBool(env, cells[1]));
                return eleveldb::ATOM_OK;
            }
            ...
            if(atom == "init_atomic_counters") {
                ...
                if(ErlUtil::isTuple(env, cells[1])) {
                    ...
                    if(args.size() == 4 && ErlUtil::isList(env, args[0])) {
                        ...
                        nifutil::Profiler::initializeAtomicCounters(nameMap, bufferSize, minorIntervalMs, outputFile);
            ...
            if(atom == "inc_atomic_counter") {
                ...
                nifutil::Profiler::incrementAtomicCounter(partPtr, counterName);
                return eleveldb::ATOM_OK;
            }
            ...
            if(atom == "add_ring_partition") {
                ...
                nifutil::Profiler::addRingPartition(partPtr, leveldbFile);
                return eleveldb::ATOM_OK;
            }
            ...
            if(atom == "start" || atom == "stop") {
                ...
                ErlNifUInt64 count = nifutil::Profiler::profile(atom, label, perThread, always);
            ...
            if(atom == "dump" || atom == "prefix") {
                ...
                nifutil::Profiler::profile(atom, ErlUtil::getAsString(env, cells[1]), true);
                return eleveldb::ATOM_OK;
            }
