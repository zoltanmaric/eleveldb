%% -------------------------------------------------------------------
%%
%% eleveldb_conn: manage a connection to LevelDB
%%
...
%% API
-export([start_link/0, stop/0, open/2, is_open/0, get/0, close/1]).

%% gen_server callbacks
-export([init/1, handle_call/3, handle_cast/2, handle_info/2,
         terminate/2, code_change/3]).

-record(state, { conn :: eleveldb:db_ref() }).
...
-spec open(string(), eleveldb:open_options()) -> {ok, eleveldb:db_ref()} | {error, term()}.
open(Dir, Config) ->
...
init([]) ->
    true = eleveldb_ets:table_ready(),
    {ok, #state{}}.

handle_call({open, Dir, Config, Caller}, _From, #state{conn=undefined}=State) ->
    {Reply, NState} =
        case eleveldb:open(Dir, Config) of
            {ok, ConnRef}=OK ->
                Monitor = erlang:monitor(process, Caller),
                true = ets:insert(eleveldb_ets, {Monitor, Caller}),
...
handle_call({close, Caller}, _From, #state{conn=ConnRef}=State) ->
    {[{Monitor, Caller}], _} = ets:match_object(eleveldb_ets, {'_', Caller}, 1),
    true = erlang:demonitor(Monitor, [flush]),
    true = ets:delete(eleveldb_ets, Monitor),
    NState = case ets:info(eleveldb_ets, size) of
                 0 ->
                     do_close(ConnRef),
...
handle_cast(stop, #state{conn=ConnRef}=State) ->
    ...
    ets:foldl(fun({Monitor, _}, _) ->
                      ...
                      ets:delete(eleveldb_ets, Monitor)
              end, true, eleveldb_ets),
    {stop, normal, State#state{conn=undefined}};
handle_cast(_Msg, State) ->
    {noreply, State}.

handle_info({'DOWN', Monitor, _, _, _}, #state{conn=ConnRef}=State) ->
    NState = case ets:lookup(eleveldb_ets, Monitor) of
                 [{Monitor, _}] ->
                     true = ets:delete(eleveldb_ets, Monitor),
                     case ets:info(eleveldb_ets, size) of
                         0 ->
                             do_close(ConnRef),
                             State#state{conn=undefined};
                         _ ->
...
do_close(undefined) ->
    ok;
do_close(ConnRef) ->
    eleveldb:close(ConnRef).


-ifdef(TEST).

-define(DATADIR, "test/eleveldb-backend").
    ...
    {spawn,
     [{setup,
       fun() ->
               ...
               EtsPid = case eleveldb_ets:start_link() of
                            {ok, Pid1} ->
                                Pid1;
...
open_one() ->
    {ok, Ref} = open("test/eleveldb-backend", [{create_if_missing, true}]),
    true = is_open(),
    close(Ref),
    false = is_open(),
    ok.
